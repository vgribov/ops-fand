    ...
    if (subsysrow)
    {
        ...
        OVSREC_FAN_FOR_EACH (row, idl)
        {
            if (row)
            {
                ...
                if (at)
                    rpm=at->keys->integer;
                vty_out (vty,"%-13s%-7s%-15s%-14s%-8" PRIi64 "%s",
                        row->name,row->speed,
...
/* Initialize ops-fand cli node.
 */
void cli_pre_init(void)
{
    /* ops-fand doesn't have any context level cli commands.
     * To load ops-fand cli shared libraries at runtime,this function reuired.
     */
}

/* Initialize ops-fand cli element.
 */
